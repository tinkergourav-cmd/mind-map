          ...
          return (
            <div key={group.id} className="mb-1">
              <button
                ...
                className="w-full flex items-center gap-1.5 px-2 py-1 bg-slate-50 hover:bg-slate-100 border-b border-slate-150 transition-colors"
              >
                <span className={`w-2 h-2 rounded-full ${groupColor.dot}`}></span>
                <span className="text-[11px] font-bold text-slate-600 uppercase tracking-wide">{group.name}</span>
                <span className="text-[10px] text-slate-400">({groupTasks.length})</span>
                <ChevronDown className={`w-3 h-3 text-slate-400 ml-auto transition-transform ${isCollapsed ? '-rotate-90' : ''}`} />
              </button>
              ...
              {colorPickerGroupId === group.id && (
                <div className="absolute top-full right-2 mt-1 bg-white rounded-lg shadow-xl border border-slate-200 p-2 z-50 flex gap-1.5 flex-wrap max-w-[180px]">
                  {GROUP_COLORS.map(color => (
                    <button key={color.value} onClick={() => { onUpdateGroup(group.id, { color: color.value }); setColorPickerGroupId(null); }} className={`w-6 h-6 rounded-full ${color.dot} border-2 border-white shadow-sm hover:scale-110 transition-transform flex items-center justify-center`} title={color.label}>
                  ...
                  return (
                    <div key={task.id} className="bg-white rounded-lg border border-slate-100 p-2.5 shadow-sm hover:shadow transition-shadow">
                      <div className="flex items-start gap-2">
                        <button onClick={() => onToggleTaskStatus(task.id)} className={`mt-0.5 w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${isCompleted ? 'bg-green-500 border-green-500' : 'border-slate-300 hover:border-slate-400'}`}>{isCompleted && <CheckSquare className="w-3 h-3 text-white" />}</button>
                        <div className="flex-1 min-w-0">
                          <input type="text" value={task.title} onChange={(e) => onUpdateTask(task.id, { title: e.target.value })} className={`w-full bg-transparent text-xs font-semibold text-slate-700 focus:outline-none focus:bg-slate-50 rounded px-1 ${isCompleted ? 'line-through text-slate-400' : ''}`} placeholder="Task title..." />
                          {linkedCard && (
                            <button onClick={() => onLocateCard(linkedCard.id)} className="mt-1 inline-flex items-center gap-1 px-1.5 py-0.5 bg-indigo-50 border border-indigo-200 rounded text-[10px] font-medium text-indigo-700 hover:bg-indigo-100 hover:border-indigo-300 transition-colors cursor-pointer" title="Locate card on canvas">
                              <Circle className="w-2 h-2" /><span className="truncate max-w-[120px]">{linkedCard.title || `Card #${linkedCard.id}`}</span>
                            </button>
                          )}
                          <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
                            <select value={task.status} onChange={(e) => onUpdateTask(task.id, { status: e.target.value })} className="text-[10px] font-medium bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 text-slate-600 focus:outline-none focus:ring-1 focus:ring-indigo-300">{STATUS_OPTIONS.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}</select>
                            <select value={task.groupId} onChange={(e) => onMoveTaskToGroup(task.id, e.target.value)} className="text-[10px] font-medium bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 text-slate-600 focus:outline-none focus:ring-1 focus:ring-indigo-300">{sortedGroups.map(g => (<option key={g.id} value={g.id}>{g.name}</option>))}</select>
                            <button onClick={() => toggleNote(task.id)} className={`p-0.5 rounded hover:bg-slate-100 transition-colors ${expandedNotes[task.id] ? 'text-indigo-600' : 'text-slate-400'}`} title="Toggle note"><MessageSquare className="w-3 h-3" /></button>
                            <button onClick={() => onDeleteTask(task.id)} className="p-0.5 rounded hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors ml-auto" title="Delete task"><X className="w-3 h-3" /></button>
                          </div>
                          {expandedNotes[task.id] && (
                            <textarea value={task.note || ''} onChange={(e) => onUpdateTask(task.id, { note: e.target.value })} placeholder="Add a note..." className="mt-1.5 w-full text-[11px] bg-slate-50 border border-slate-200 rounded p-1.5 text-slate-600 placeholder-slate-400 resize-none focus:outline-none focus:ring-1 focus:ring-indigo-300 min-h-[40px]" rows={2} />
                          )}
                        </div>
                      </div>
                    </div>
